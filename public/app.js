  ...
  try {
    ...
    if (typeof getAllDownloadedComicIds === 'function') {
      try {
        ...
        console.warn('Failed to load downloaded comic IDs:', error);
      }
    }
        ...
        if (typeof mergeOfflineStatusesIntoLibrary === 'function') {
          ...
          if (typeof saveLibraryCacheToDB === 'function') {
            saveLibraryCacheToDB(library).catch(error => {
              console.warn('Failed to update library cache after progress merge:', error);
            });
          }
        }
      }
...
function showUserBadge() {
  ...
  console.log('[AUTH] User badge displayed:', { email: userEmail, role: userRole });
}
