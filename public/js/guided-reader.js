...
(function () {
  ...
  function entryHtml(entry) {
    ...
    const idAttr = entry.id ? ` data-log-id="${escapeHtml(entry.id)}"` : '';
    return `<div class="log-entry"${idAttr}><span class="log-${level}">${level}</span> ${escapeHtml(entry.message || '')}</div>`;
  }
  ...
  window.openGuidedReaderTab = function () {
    bindOnce();
    fetchStatus();
    startStream();
    startPolling();
  ...
  const origClose = window.closeSettingsModal;
