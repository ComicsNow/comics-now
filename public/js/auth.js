...
function handleLogout() {
  if (confirm('Are you sure you want to log out?')) {
    ...
    if (window.offlineManager) {
      window.offlineManager.clearAllDownloads().catch(err => {
        console.error('[AUTH] Failed to clear offline data:', err);
      });
    }
    ...
    if (teamDomain) {
      ...
      console.warn('[AUTH] Cloudflare team domain not configured');
