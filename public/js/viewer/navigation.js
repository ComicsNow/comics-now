(function (global) {
  ...
  async function loadOfflineComicPages(comic) {
    ...
    try {
      ...
      if (!JSZipCtor) {
        console.warn('JSZip is not available; cannot load offline comic pages.');
        return [];
      }
      ...
      console.error('Failed to load offline comic pages:', error);
      return [];
    }
  }
  ...
  async function openComicViewer(comic) {
    ...
    if (isOffline || isDownloaded) {
      try {
        ...
        if (offlinePages.length > 0) {
          ...
          console.warn('Offline but no downloaded copy of this comic was found. Falling back to network fetch.');
        }
      } catch (offlineError) {
        console.warn('Failed to load offline comic pages before network request:', offlineError);
      }
    }
    ...
    if (!Array.isArray(pages) || pages.length === 0) {
      try {
        ...
        if (Array.isArray(offlineFallback) && offlineFallback.length > 0) {
          ...
          console.warn('Falling back to offline comic pages due to fetch failure:', error);
        } else {
    ...
    if (global.window.syncManager) {
      try {
        ...
        console.warn('[Viewer] Sync check failed:', error);
      }
    }
    ...
    if (typeof global.loadComicSummary === 'function') {
      try {
        ...
        console.warn('[Viewer] Failed to load comic summary:', error);
      }
    }
