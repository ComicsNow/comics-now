...
class SyncManager {
  ...
  loadIgnoredSyncPoints() {
    try {
      ...
      console.warn('Failed to load ignored sync points:', error);
      return {};
    }
  }
  ...
  saveIgnoredSyncPoints() {
    try {
      ...
      console.warn('Failed to save ignored sync points:', error);
    }
  }
  ...
  async initialize() {
    try {
      ...
      if (userResponse.ok) {
        ...
        console.warn('[SYNC] Failed to fetch user info, using defaults');
      }
    } catch (error) {
      console.warn('[SYNC] Failed to load user context:', error);
  ...
  generateFingerprint() {
    try {
      ...
      try {
        ...
        debugLog('SYNC', 'Canvas fingerprinting failed (may be blocked):', canvasError.message);
      }
    ...
    try {
      ...
      if (savedDeviceId && savedDeviceName) {
        ...
        try {
          ...
          console.warn('[SYNC] Could not verify cached device, using anyway:', verifyError.message);
          this.deviceId = savedDeviceId;
          this.deviceName = savedDeviceName;
          return { deviceId: this.deviceId, deviceName: this.deviceName };
        }
      ...
      console.error('Device initialization failed:', {
        error: error.message,
        stack: error.stack,
        deviceName: this.generateAutoDeviceName()
      });
      throw error;
    }
  }
  ...
  async showDeviceSelectionDialog(comicId, currentPage, syncData) {
    try {
      ...
      console.error('Failed to show device selection dialog:', error);
      return { shouldSync: false, error: error.message };
    }
  }
      ...
      if (response.ok) {
        ...
        if (typeof saveLibraryCacheToDB === 'function' && typeof library !== 'undefined') {
          saveLibraryCacheToDB(library).catch(error => {
            console.warn('Failed to update library cache after progress update:', error);
          });
        }
      }
  ...
  clearStoredDevice() {
    try {
      ...
      console.warn('Failed to clear stored device info:', error);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(async () => {
    try {
      ...
      console.error('[SYNC] Sync initialization failed:', {
        error: error.message,
        stack: error.stack
      });
