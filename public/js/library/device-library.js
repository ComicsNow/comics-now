 ...
 */
export async function requestDeviceLibrary() {
 ...
 */
export async function saveLegacyDeviceLibrary(id, name, files) {
  const db = state.OfflineDB || window.OfflineDB || {};
  if (!db.openOfflineDB) throw new Error('OfflineDB not initialized');
  const activeDb = await db.openOfflineDB();
  ...
  // Save to IndexedDB
  const tx = activeDb.transaction([DEVICE_LIBRARIES_STORE, DEVICE_COMICS_STORE], 'readwrite');
 ...
 */
export async function saveDeviceLibrary(id, handle) {
  const db = state.OfflineDB || window.OfflineDB || {};
  if (!db.openOfflineDB) throw new Error('OfflineDB not initialized');
  const activeDb = await db.openOfflineDB();

  return new Promise((resolve, reject) => {
    const tx = activeDb.transaction([DEVICE_LIBRARIES_STORE], 'readwrite');
 ...
 */
export async function scanDeviceLibrary(libraryId, directoryHandle) {
  ...
  const tx = activeDb.transaction([DEVICE_COMICS_STORE], 'readwrite');
 ...
 */
export async function getDeviceLibraries() {
  const db = state.OfflineDB || window.OfflineDB || {};
  if (!db.openOfflineDB) throw new Error('OfflineDB not initialized');
  const activeDb = await db.openOfflineDB();

  if (!activeDb.objectStoreNames.contains(DEVICE_LIBRARIES_STORE)) return [];

  return new Promise((resolve, reject) => {
    const tx = activeDb.transaction([DEVICE_LIBRARIES_STORE], 'readonly');
    const store = tx.objectStore(DEVICE_LIBRARIES_STORE);
 ...
 */
export async function getDeviceComics(libraryId) {
  const db = state.OfflineDB || window.OfflineDB || {};
  if (!db.openOfflineDB) throw new Error('OfflineDB not initialized');
  const activeDb = await db.openOfflineDB();

  if (!activeDb.objectStoreNames.contains(DEVICE_COMICS_STORE)) return [];

  return new Promise((resolve, reject) => {
    const tx = activeDb.transaction([DEVICE_COMICS_STORE], 'readonly');
    const store = tx.objectStore(DEVICE_COMICS_STORE);
    const request = store.getAll();
    request.onsuccess = () => {
 ...
 */
export async function removeDeviceLibrary(libraryId) {
  const db = state.OfflineDB || window.OfflineDB || {};
  if (!db.openOfflineDB) throw new Error('OfflineDB not initialized');
  const activeDb = await db.openOfflineDB();
 ...
 */
export async function appendDeviceLibraryCards(container) {
  ...
  for (const lib of libraries) {
    ...
    card.innerHTML = `
      <div class="aspect-[2/3] bg-gray-700 flex flex-col items-center justify-center p-4 text-center">
        ...
        <div class="text-white font-bold truncate w-full">${escapeHtml(lib.name)}</div>
 ...
 */
export async function reattachLegacyLibrary(libraryId) {
  const input = document.createElement('input');
  input.type = 'file';
  input.multiple = true;
 ...
 */
export async function showDeviceLibraryComics(libraryId, libraryName) {
  const comics = await getDeviceComics(libraryId);
...
export function renderLocalComicList(title, comics) {
  ...
  if (typeof showView === 'function') {
    showView(document.getElementById('comic-list'));
  }
    ...
    card.innerHTML = `
      <div class="aspect-[2/3] bg-gray-700 flex items-center justify-center relative overflow-hidden">
        ...
        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2">
          <div class="text-white text-xs font-bold truncate">${escapeHtml(comic.name)}</div>
        </div>
      </div>
    `;

    card.onclick = () => {
      const openComicViewer = state.openComicViewer || window.openComicViewer;
      if (typeof openComicViewer === 'function') {
        openComicViewer(comic);
      }
    };
...
state.DeviceLibrary = DeviceLibrary;
Object.assign(state, DeviceLibrary);

if (typeof window !== 'undefined') {
  window.DeviceLibrary = DeviceLibrary;
  Object.assign(window, DeviceLibrary);
}
