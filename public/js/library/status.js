...
export function getComicStatus(comic) {
...
export function getSeriesStatus(comicsInSeries) {
...
export function getComicStatusCounts(comicsInSeries) {
...
export function addStatusCounts(target, addition) {
  if (!target) {
...
export function getPublisherStatusCounts(publisherData = {}) {
  // Pre-computed counts don't know about smart scope; bypass when a scope is active.
  const activeSmartFilter = state.activeSmartFilter || window.activeSmartFilter;
  if (!activeSmartFilter && publisherData._counts) {
...
export function getRootStatusCounts(rootData = {}) {
  const counts = createEmptyStatusCounts();
...
export function getLibraryStatusCounts(libraryData = {}) {
  const counts = createEmptyStatusCounts();
...
export function statusCountsMatchFilter(counts = createEmptyStatusCounts()) {
...
export function filterPublishersByActiveFilter(publishers = {}) {
  const scope = state.activeSmartFilter || window.activeSmartFilter || null;
  const activeFilter = state.activeFilter || window.activeFilter || 'all';
...
export function filterSeriesByActiveFilter(seriesEntries = {}) {
  const scope = state.activeSmartFilter || window.activeSmartFilter || null;
  const activeFilter = state.activeFilter || window.activeFilter || 'all';
  if (activeFilter === 'all' && !scope) {
...
export function updateFilterButtonCounts() {
...
export function updateLibraryReadStatus({ rootFolder, publisher, seriesName, comicId, status }) {
  const library = state.library || window.library;
  if (!library) return;

  // Invalidate folder cache if it exists
  const invalidateFolderCache = state.invalidateFolderCache || window.invalidateFolderCache;
  ...
  const applyStatusToComic = (comic) => {
    ...
    if (typeof SmartLists.syncDownloadedComicStatusFromLibrary === 'function') {
  ...
  const renderDownloadedSmartList = state.renderDownloadedSmartList || window.renderDownloadedSmartList || (state.LibrarySmartFilters?.renderDownloadedSmartList) || (window.LibrarySmartFilters?.renderDownloadedSmartList);
...
Object.assign(state, LibraryStatus);

state.getComicStatus = getComicStatus;
state.getSeriesStatus = getSeriesStatus;
state.getComicStatusCounts = getComicStatusCounts;
state.updateLibraryReadStatus = updateLibraryReadStatus;

if (typeof window !== 'undefined') {
  window.LibraryStatus = LibraryStatus;
  Object.assign(window, LibraryStatus);
  
  window.getComicStatus = getComicStatus;
  window.getSeriesStatus = getSeriesStatus;
  window.getComicStatusCounts = getComicStatusCounts;
  window.updateLibraryReadStatus = updateLibraryReadStatus;
}
