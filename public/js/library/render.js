...
function showPublisherList(rootFolder, options = {}) {
  ...
  if (!library || !library[rootFolder]) {
    console.warn(`Root folder ${rootFolder} not found in library.`);
...
function showSeriesList(publisherName, options = {}) {
  ...
  if (!publisherData) {
    console.warn(`Publisher ${publisherName} not found under ${currentRootFolder}.`);
    return;
  }
...
async function showComicList(seriesName) {
  ...
  try {
    ...
    console.error('Failed to load comics for series:', seriesName, error);
    if (container) {
...
// ...
    ...
    card.addEventListener('click', (event) => {
      ...
      console.debug('[Library] Comic card click detected', clickDebugDetails);

      if (target instanceof Element) {
        const interactiveButton = target.closest('button.download-btn, button.status-toggle-btn');
        if (interactiveButton) {
          console.debug('[Library] Click was on comic card interactive control; viewer will not open.', {
            ...clickDebugDetails,
            controlType: interactiveButton.classList?.contains('download-btn')
              ? 'download'
              : interactiveButton.classList?.contains('status-toggle-btn')
                ? 'status-toggle'
                : 'unknown',
          });
          return;
      ...
      console.debug('[Library] Opening comic viewer from card click.', clickDebugDetails);
      if (typeof window.openComicViewer === 'function') {
        window.openComicViewer(comic);
      } else {
        console.error('[Library] openComicViewer function not available');
      }
    });
