...
async function applyStatus(comics, status, progressEl) {
  ...
  for (const comic of comics) {
    try {
      ...
      const updateLibraryReadStatus = state.updateLibraryReadStatus || window.updateLibraryReadStatus;
      if (typeof updateLibraryReadStatus === 'function') {
        updateLibraryReadStatus({ comicId: comic.id, status });
      }
    } catch (_) {
...
export function open(comics, scopeLabel) {
...
state.openBulkReadStatusDialog = open;
state.closeBulkReadStatusDialog = close;

if (typeof window !== 'undefined') {
  window.openBulkReadStatusDialog = open;
  window.closeBulkReadStatusDialog = close;
}
