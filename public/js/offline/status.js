(function (global) {
  ...
  async function saveStatusToDB(item) {
    ...
    return new Promise((resolve, reject) => {
      const tx = global.db.transaction(['statuses'], 'readwrite');
  ...
  async function updateOfflineReadStatus(comicId, status) {
    ...
    return new Promise(resolve => {
      const tx = global.db.transaction(['comics', 'progress'], 'readwrite');
  ...
  async function mergeOfflineStatusesIntoLibrary() {
    ...
    return new Promise((resolve) => {
      const tx = global.db.transaction(['statuses', 'progress'], 'readonly');
  ...
  async function syncOfflineProgress() {
    ...
    progressSyncPromise = (async () => {
      ...
      const unsynced = await new Promise((resolve, reject) => {
        const tx = global.db.transaction(['progress'], 'readonly');
        ...
        try {
          ...
          if (!comicPath) {
            ...
            await new Promise((resolve, reject) => {
              const tx2 = global.db.transaction(['progress'], 'readwrite');
  ...
  async function syncOfflineStatuses() {
    ...
    const unsynced = await new Promise((resolve, reject) => {
      const tx = global.db.transaction(['statuses'], 'readonly');
      const store = tx.objectStore('statuses');
      const req = store.getAll();
      req.onsuccess = () => resolve((req.result || []).filter(i => !i.synced));
      req.onerror = () => reject(req.error || new Error('getAll failed'));
