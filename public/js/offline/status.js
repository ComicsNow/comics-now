(function (global) {
  ...
  async function updateOfflineSeriesStatus(rootFolder, publisher, seriesName, status) {
    ...
    if (Array.isArray(series)) {
      ...
      console.warn('Cannot update series status - series not loaded yet');
    }
  }
        ...
        try {
          ...
          if (window.syncManager && window.syncManager.deviceId) {
            try {
              ...
              console.log(`[OFFLINE-SYNC] Synced progress for comic ${item.id} to page ${pageValue}`);
            } catch (syncError) {
              console.error(`Failed to sync progress via syncManager for comic ${item.id}:`, syncError);
              continue;
            }
          } else {
            console.warn(`[OFFLINE-SYNC] Sync manager not available, skipping progress sync for comic ${item.id}`);
            continue;
          }
          ...
          console.error(`Failed to sync progress for comic ${item.id}:`, error);
        }
      }
    })();
  ...
  async function syncOfflineStatuses() {
    ...
    for (const item of unsynced) {
      ...
      try {
        ...
        console.error(`Failed to sync status for ${item.key}:`, error);
      }
    }
  }
  ...
  async function enhancedBackgroundSync() {
    try {
      console.log('Starting enhanced background sync...');

      if (navigator.onLine === false) {
        console.log('Offline - skipping network sync operations');
        return;
      }
      ...
      if (typeof loadLibraryCacheFromDB === 'function') {
        try {
          ...
          console.warn('Failed to read library cache timestamp from IndexedDB:', error);
        }
      }
      ...
      if (cacheAge > 5 * 60 * 1000) {
        console.log('Library cache is stale, refreshing...');
        if (typeof global.fetchLibraryFromServer === 'function') {
          await global.fetchLibraryFromServer();
        }
      }

      console.log('Enhanced background sync completed');
    } catch (error) {
