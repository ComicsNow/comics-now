(function (global) {
  ...
  async function openOfflineDB() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = indexedDB.open('comics-now-offline', 7);

      request.onerror = (event) => {
        console.error('[PROGRESS] Error opening IndexedDB:', event.target.error);
  ...
  async function saveLibraryCacheToDB(libraryData) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        request.onerror = (event) => {
          console.warn('Failed to save library cache to IndexedDB:', event.target.error);
          reject(event.target.error);
        };
      } catch (error) {
        console.warn('Exception while saving library cache to IndexedDB:', error);
        reject(error);
      }
    });
  }
  ...
  async function loadLibraryCacheFromDB() {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        request.onerror = (event) => {
          console.warn('Failed to read library cache from IndexedDB:', event.target.error);
          reject(event.target.error);
        };
      } catch (error) {
        console.warn('Exception while reading library cache from IndexedDB:', error);
        reject(error);
      }
    });
  }
  ...
  async function clearLibraryCacheFromDB() {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        request.onerror = (event) => {
          console.warn('Failed to clear library cache from IndexedDB:', event.target.error);
          reject(event.target.error);
        };
      } catch (error) {
        console.warn('Exception while clearing library cache from IndexedDB:', error);
        reject(error);
      }
    });
  }
  ...
  async function getComicFromDB(comicId) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        request.onerror = (event) => {
          console.error(`[PROGRESS] getComicFromDB error for ID ${comicId}:`, event.target.error);
          reject(new Error(`Failed to get comic: ${event.target.errorCode}`));
        };
      } catch (error) {
        console.error(`[PROGRESS] Exception in getComicFromDB for ID ${comicId}:`, error);
        reject(error);
      }
    });
  }
  ...
  async function deleteOfflineComic(comicId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      function proceedWithDeletion() {
        ...
        tx.oncomplete = () => {
          ...
          if (comicDataFound && comicDataFound.fileBlob) {
            try {
              ...
              console.warn('Error cleaning up comic blob:', error);
            }
          }
        ...
        tx.onerror = (event) => {
          console.error(`Failed to delete comic ${comicId}:`, event.target.error);
  ...
  async function getOfflineComicRecordById(targetId) {
    ...
    if (!db) {
      try {
        ...
        console.warn('[PROGRESS] Unable to open IndexedDB while resolving comic path:', error);
        return null;
      }
    }
      ...
      try {
        const record = await new Promise(resolve => {
          try {
            ...
            console.warn('[PROGRESS] Failed to query IndexedDB for comic key', key, error);
            resolve(null);
          }
        });
        if (record) return record;
      } catch (error) {
        console.warn('[PROGRESS] IndexedDB lookup failed for comic', targetId, error);
      }
    }
