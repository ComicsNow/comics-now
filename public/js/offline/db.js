(function (global) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        debugLog(
          ...
          Array.from(global.db.objectStoreNames)
        );
        resolve(global.db);
      };
  ...
  async function saveLibraryCacheToDB(libraryData) {
    ...
    if (!global.db) await openOfflineDB();

    if (!global.db.objectStoreNames.contains(LIBRARY_CACHE_STORE)) {
    ...
    return new Promise((resolve, reject) => {
      try {
        const tx = global.db.transaction([LIBRARY_CACHE_STORE], 'readwrite');
  ...
  async function loadLibraryCacheFromDB() {
    ...
    return new Promise((resolve, reject) => {
      try {
        const tx = global.db.transaction([LIBRARY_CACHE_STORE], 'readonly');
        const store = tx.objectStore(LIBRARY_CACHE_STORE);
  ...
  async function deleteFromCache(comicId, comicPath) {
    ...
    try {
      ...
      for (const request of keys) {
        ...
        // 1. Match by ID in API URLs (with boundary check to avoid partial matches)
        // Matches /api/comics/ID or /api/v1/comics/ID followed by /, ?, or end of string
        const apiPattern = new RegExp(`\\/api\\/(?:v1\\/)?comics\\/${escapedId}(?:$|\\/|\\?)`);
        
        if (apiPattern.test(url)) {
          shouldDelete = true;
        } 
        // 2. Match by encoded path in page image URLs
        else if (encodedPath && url.includes('path=') && url.includes(encodeURIComponent(encodedPath))) {
          shouldDelete = true;
        }
        // 3. Match by thumbnail URL
  ...
  async function deleteOfflineComic(comicId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      function proceedWithDeletion() {
        ...
        tx.oncomplete = async () => {
          // Also clear the Cache API data for this comic
          // Pass the path if available to ensure all cached assets (like pages) are removed
