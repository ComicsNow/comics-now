    ...
    try {
      ...
      const updateNextComicRecord = (index = 0) => {
        ...
        try {
          ...
          console.warn('[PROGRESS] Failed to open comic record for update:', error);
        ...
        request.onsuccess = () => {
          ...
          if (comicData && comicData.comicInfo) {
            ...
            try {
              ...
              console.error('[PROGRESS] Failed to persist comic progress update:', error);
            }
          } else {
            updateNextComicRecord(index + 1);
          }
      ...
      transaction.onerror = (event) => {
        console.error('[PROGRESS] IndexedDB transaction failed:', event.target.error);
        reject(new Error('IndexedDB transaction failed: ' + event.target.error));
      };
    } catch (error) {
      console.error('[PROGRESS] Exception while saving progress to IndexedDB:', error);
      reject(error);
    }
  });
}
...
async function updateOfflineProgressForComic(comicId, status) {
  ...
  return new Promise((resolve) => {
    ...
    request.onerror = (event) => {
      console.warn('Failed to update offline progress:', event.target.errorCode);
...
async function updateOfflineProgressForSeries(rootFolder, publisher, seriesName, status) {
  ...
  if (Array.isArray(seriesComics)) {
    ...
    console.warn('Cannot update series status - series not loaded yet. Loading series first...');
  ...
  try {
    ...
    if (typeof saveLibraryCacheToDB === 'function' && typeof library !== 'undefined') {
      saveLibraryCacheToDB(library).catch(error => {
        console.warn('Failed to update library cache after status change:', error);
      });
    }
