(function (global) {
  ...
  function showComicContextMenu(event, comic) {
    ...
    readStatusItem.addEventListener('click', async (e) => {
      ...
      if (typeof global.toggleReadStatus === 'function') {
        try {
          ...
          // Directly update the comic object so the next right-click shows the correct
          // status — updateLibraryReadStatus can fail silently on lazy-loaded series placeholders
    ...
    if (isEligibleComic(comic)) {
      const isProcessed = comic.guidedViewStatus === 'completed';
      const isProcessing = comic.guidedViewStatus === 'processing';
      const runItem = document.createElement('div');
      runItem.className = 'comic-context-menu-item';
      runItem.innerHTML = `${ICONS.BOOK}<span>${isProcessed ? 'Re-run' : 'Run'} Guided Detection</span>`;
      if (isProcessing) {
        runItem.style.cursor = 'default';
        runItem.style.opacity = '0.6';
        runItem.querySelector('span').textContent = 'Guided Detection Running…';
      } else {
        runItem.addEventListener('click', async (e) => {
          e.preventDefault();
          e.stopPropagation();
          closeContextMenu();
          await triggerGuidedRunForScope('comic', comic.id, comic.name || 'this comic');
        });
      }
      menu.appendChild(runItem);
    }

    // 6. Reading List menu item
  ...
  function showSeriesContextMenu(event, seriesData) {
    ...
    // 1b. Bulk Read/Unread dialog — same flow as bulk download (count + confirm).
    if (Array.isArray(comicsInSeries) && comicsInSeries.length > 0) {
      const bulkReadItem = document.createElement('div');
      bulkReadItem.className = 'comic-context-menu-item';
      bulkReadItem.innerHTML = `${ICONS.EYE}<span>Bulk Read/Unread…</span>`;
      bulkReadItem.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        closeContextMenu();
        if (typeof global.openBulkReadStatusDialog === 'function') {
          global.openBulkReadStatusDialog(comicsInSeries, `series "${seriesName}"`);
        }
      });
      menu.appendChild(bulkReadItem);
    }
    ...
    // 4b. Run Guided Detection for all comics in the series.
    if (Array.isArray(comicsInSeries)) {
      const eligibleCount = comicsInSeries.filter(isEligibleComic).length;
      if (eligibleCount > 0) {
        const runItem = document.createElement('div');
        runItem.className = 'comic-context-menu-item';
        runItem.innerHTML = `${ICONS.BOOK}<span>Run Guided Detection (${eligibleCount})</span>`;
        runItem.addEventListener('click', async (e) => {
          e.preventDefault();
          e.stopPropagation();
          closeContextMenu();
          await triggerGuidedRunForScope('series', seriesName, `series '${seriesName}'`);
        });
        menu.appendChild(runItem);
      }
    }
  ...
  function showPublisherContextMenu(event, publisherData) {
    ...
    // 0. Bulk Read/Unread dialog for all comics under this publisher
    if (allComics.length > 0) {
      ...
      bulkReadItem.addEventListener('click', (e) => {
        ...
        closeContextMenu();
        if (typeof global.openBulkReadStatusDialog === 'function') {
          global.openBulkReadStatusDialog(allComics, `publisher "${publisherName}"`);
        }
      });
      menu.appendChild(bulkReadItem);
    }
    ...
    // 3b. Run Guided Detection for all comics under this publisher.
    {
      const eligibleCount = allComics.filter(isEligibleComic).length;
      if (eligibleCount > 0) {
        const runItem = document.createElement('div');
        runItem.className = 'comic-context-menu-item';
        runItem.innerHTML = `${ICONS.BOOK}<span>Run Guided Detection (${eligibleCount})</span>`;
        runItem.addEventListener('click', async (e) => {
          ...
          e.stopPropagation();
          closeContextMenu();
          await triggerGuidedRunForScope('publisher', publisherName, `publisher '${publisherName}'`);
        });
        menu.appendChild(runItem);
      }
    }
  ...
  function showLibraryContextMenu(event, libraryData) {
    ...
    // 0. Bulk Read/Unread dialog for the entire library
    if (allComics.length > 0) {
      ...
      bulkReadItem.addEventListener('click', (e) => {
        ...
        closeContextMenu();
        if (typeof global.openBulkReadStatusDialog === 'function') {
          global.openBulkReadStatusDialog(allComics, `library "${libLabel}"`);
        }
      });
      menu.appendChild(bulkReadItem);
    }
    ...
    // 3b. Run Guided Detection for all comics in this library.
    {
      const eligibleCount = allComics.filter(isEligibleComic).length;
      if (eligibleCount > 0) {
        const runItem = document.createElement('div');
        ...
        runItem.addEventListener('click', async (e) => {
          ...
          e.stopPropagation();
          closeContextMenu();
          const label = `library '${folderPath || 'root'}'`;
          await triggerGuidedRunForScope('library', folderPath, label);
        });
        menu.appendChild(runItem);
      }
    }
