...
function initJwksClient() {
  ...
  if (!teamDomain) {
    console.warn('⚠️  Cloudflare team domain not configured');
    return null;
  }
  ...
  if (!jwksClientInstance) {
    ...
    console.log(`✓ JWKS client initialized for ${teamDomain}`);
  }
...
async function extractUserFromJWT(req, res, next) {
  ...
  if (!isAuthEnabled()) {
    ...
    if (testUserId) {
      ...
      console.log(`[AUTH] MOCK USER: ${userId} (${email}, ${role})`);
    }
  ...
  if (!jwtToken) {
    ...
    if (trustedIPs.length > 0) {
      ...
      console.log(`[IP CHECK] Client IP: ${clientIP}, Normalized: ${normalizedIP}, Trusted IPs: ${trustedIPs.join(', ')}`);

      if (isIPInTrustedList(normalizedIP, trustedIPs)) {
        console.log(`✓ Trusted IP bypass: ${normalizedIP}`);
        req.user = {
          userId: 'default-user',
        ...
        try {
          ...
          console.error('Failed to create/update local user:', error);
        }
