...
async function runComicTagger() {
  ...
  try {
    ...
    for (const entry of entries) {
      if (!entry.isFile() || path.extname(entry.name).toLowerCase() !== '.cbz') continue;
        ...
        const handleLine = (l) => {
          ...
          if (/Choose a match.*or.*skip/i.test(l)) {
            if (!awaitingUserInput) {
              ...
              ctLog('[CT] >>> WAITING FOR USER SELECTION <<<');
              ...
              pendingUserChoice.then((choice) => {
                ...
                ctLog(`[CT] Error handling user choice: ${err.message}`);
                pendingMatchState = null;
              });
            }
          }
        ...
        currentProcess.on('close', async (code) => {
          ...
          if (isSuccessful) {
            ...
            try {
              ...
              ctLog(`[CT] ➜ YES: ${entry.name}`);
            } catch (err) {
              ctLog(`[CT] Move failed for ${entry.name}: ${err.message}`);
            }
          } else {
            ctLog(`[CT] ➜ KEEP: ${entry.name}`);
          }
          currentProcess = null;
    ...
    ctLog('ComicTagger run completed.');
  } catch (err) {
