...
function createApiRouter({
  ...
  router.get('/api/v1/search/comicvine', async (req, res) => {
    try {
      ...
      for (const item of (sdata.results || [])) {
        ...
        if (type === 'volume') {
          try {
            ...
            if (!matchesVolume(ij.results, volIdNum)) {
              console.warn('[CV VolumeIssues] filter ignored by API', { volIdNum, issuesTried: issuesUrl });
              ij.results = [];
            }
            ...
            console.log('[CV VolumeIssues]', {
              volume: obj.name,
              volIdNum,
              usedFilter: issuesUrl,
              ids: Array.isArray(obj.issues) ? obj.issues.slice(0, 3).map(i => i.id) : []
            });
          } catch (err) {
            console.warn(`[CV Volume] enrich failed for ${item.id}:`, err.message);
          }
        }
      ...
      console.error('ComicVine search error:', e);
  ...
  router.get('/api/v1/comicvine/volume/:id', async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn('[CV Volume] creator enrich failed:', enrichErr.message);
      }

      return res.json(normalized);
    } catch (e) {
      console.error('ComicVine volume error:', e);
    ...
    try {
      ...
      if (!publisher && issue?.volume?.id) {
        ...
        try {
          ...
          console.error('[CV] Volume fetch failed', err.status, volUrl);
        }
      }
      ...
      console.error('ComicVine issue error:', e);
